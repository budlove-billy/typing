...
func _build() -> void:
    ...
    intro_box.add_child(GameTools.label("SOUND  |  RHYTHM", 10, ThemeKit.AMBER))
    intro_box.add_child(GameTools.label(I18n.t("rhythm_ready"), 15, ThemeKit.INK, HORIZONTAL_ALIGNMENT_LEFT, true))
    var row := HBoxContainer.new()
    root.add_child(row)
    status = GameTools.label(I18n.t("rhythm_listen"), 13, ThemeKit.MUTED, HORIZONTAL_ALIGNMENT_LEFT, true)
    status.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    row.add_child(status)
    round_label = GameTools.pill("01 / 05", ThemeKit.AMBER_SOFT, ThemeKit.AMBER)
    ...
    for index in range(PADS):
        ...
        pad.add_theme_font_size_override("font_size", 28)
        pad.pressed.connect(_pad_pressed.bind(index, pad))
        pads_grid.add_child(pad)
        pads.append(pad)
    root.add_child(GameTools.label(I18n.t("rhythm_hint"), 10, ThemeKit.SUBTLE, HORIZONTAL_ALIGNMENT_CENTER))
...
func _new_round() -> void:
    ...
    status.text = I18n.t("rhythm_listen")
...
func _play_pattern() -> void:
    for value in sequence:
        ...
        GameTools.animate_press(pads[value])
        await get_tree().create_timer(0.27).timeout
    status.text = I18n.t("rhythm_tap")
    accepting = true
    for pad in pads:
        pad.disabled = false
...
func _pad_pressed(index: int, pad: Button) -> void:
    ...
    if index != sequence[input_position]:
        ...
        mistakes += 1
        AudioDirector.bad()
        status.text = I18n.t("rhythm_wrong")
        ...
        return
    input_position += 1
    status.text = I18n.t("rhythm_good")
