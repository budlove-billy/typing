...
func _build() -> void:
    ...
    intro_box.add_child(GameTools.label("SIGHT  |  COLOR", 10, ThemeKit.BLUE))
    intro_box.add_child(GameTools.label(I18n.t("spot_ready"), 15, ThemeKit.INK, HORIZONTAL_ALIGNMENT_LEFT, true))
    var row := HBoxContainer.new()
    root.add_child(row)
    status = GameTools.label(I18n.t("spot_round_ready"), 13, ThemeKit.MUTED, HORIZONTAL_ALIGNMENT_LEFT, true)
    ...
    root.add_child(grid)
    root.add_child(GameTools.label("FIND THE ONE SLIGHTLY DIFFERENT COLOR", 10, ThemeKit.SUBTLE, HORIZONTAL_ALIGNMENT_CENTER))

func _new_round() -> void:
    locked = false
    target_index = rng.randi_range(0, 15)
    var base := Color("#6480ef")
    var different := base.lightened(0.12 + float(round_index) * 0.005)
    round_label.text = "%02d / %02d" % [round_index + 1, ROUNDS]
    status.text = I18n.t("spot_round_ready")
    status.add_theme_color_override("font_color", ThemeKit.MUTED)
    for child in grid.get_children():
        child.queue_free()
    for index in range(16):
        var fill := different if index == target_index else base
        var button: Button = GameTools.button("", fill, ThemeKit.BLUE_DARK, 70, ThemeKit.BLUE)
...
func _tap(index: int, button: Button) -> void:
    ...
    if index == target_index:
        correct_count += 1
        AudioDirector.good()
        status.text = I18n.t("spot_good")
        status.add_theme_color_override("font_color", ThemeKit.TEAL)
    else:
        mistakes += 1
        AudioDirector.bad()
        status.text = I18n.t("spot_wrong")
