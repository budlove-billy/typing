...
## 웹 품질 기준선 (2026-09-08)
- 정밀 보완: `bt_loadBest` 결과에는 비직렬화 storageKey를 붙인다. `bt_recordScore`는 판정 전 최신 기록을 읽고 `bt_saveBest`는 저장 전 전체/일일 최고점을 병합해 백업·다른 탭 기록을 보호한다. 동일 날짜 백업 병합도 day 최댓값을 유지한다.
- 회귀 범위: 기록 손실 재현, 366일×3언어 미션, flash의 종료/저장/재도전, 실제 서비스워커 온라인→오프라인 홈/미방문 페이지. 모든 게임 완주 자동화는 아직 아니다. 모아모아 신규 문제 제작은 별도이며 현재 30개 순환을 명시한다.
...
- APK verification passed with Android `apksigner` v2/v3 and SHA-256 `A16C0F9A5469C9EE2EAD54FF37FBBCFB2698232BB7EBDCA57C222FF6C3A6DDFB`. A release APK/AAB still needs a project-owned non-debug keystore; never publish the debug keystore.
...
- `mobile/godot/run-android-emulator.ps1` is the repeatable boot, non-incremental streamed install, and direct activity-launch entry point. It targets only `emulator-5554`, so a connected physical device is not touched.

## Godot mobile UI system (2026-08-05)
- `ThemeKit.gd` owns the mobile palette, padded surfaces, subtle borders/shadows, and button states. Main app screens and all five native games use these shared tokens.
...
- `ReactGame.gd` is the fourth native game: five-round blue-signal reaction test with false-start penalty, reaction-time score, result/record integration, and Korean/English strings. `SwitchGame.gd` is the fifth: six-round shape/color rule-switching test with accuracy and reaction-time scoring. Home keeps a three-game daily routine while Games/Records expose all five.
- Android 15 emulator visual QA passed for the first-launch assessment intro, memory, focus, calculation, coordination, assessment result, Home, Games, Records, Flash, Bubble, Trace, React, and Switch at 1080x2400. Final reference captures are under `generated_images/playmallow-*.png`.
...
- Native quality pass adds `AmbientBackdrop.gd`, slime idle breathing, pulsing assessment targets, and tap/feedback easing in Flash, Bubble, Trace, React, Switch, and assessment controls.

## Native game continuation (2026-08-05)
- Added `switch` / 규칙 바꾸기 as a focus-axis native game. Six rounds alternate the shape rule and color rule every two rounds; each round accepts left/right touch input and scores both accuracy and response speed.
- The game uses a code-drawn glossy stimulus tile, breathing glow, correct/incorrect tint feedback, and answer-button easing so it matches the existing slime UI without adding a fragile bitmap asset.
- `GameCatalog`, `SaveStore`, Korean/English strings, result records, daily recommendation ranking, and headless smoke coverage all include the new game. Android 15 visual QA passed on the rebuilt APK, including the first round and the shape-to-color rule transition.
...
- Verified with Godot 4.7.1 headless editor import, main-scene run, and `tests/smoke.gd` instantiation checks for all three games. The headless environment still reports a non-fatal root certificate-store warning.
- Generated Godot import/editor data is ignored. Android Studio/JDK/SDK/ADB and real-device QA remain before APK/AAB export.
...
- 첫 vertical slice는 `flash`(기억/카드), `bubble`(계산/탭), `trace`(협응/드래그) + 홈·결과·기록·설정·로컬 저장·사운드·햅틱·Android 생명주기다. 이후 게임 묶음 단위로 확장한다.
- 기존 웹 기록은 `MLW1.` 백업 코드를 네이티브에서 가져오는 경로를 공식 마이그레이션으로 삼고, Godot `user://` 버전 JSON 저장 계약을 사용한다.
- 관련 실행 문서: `모바일-네이티브-전환-검토.md`. 다음 착수 조건은 Godot/Android Studio/JDK 17/Android SDK·ADB 설치 및 빈 앱의 실기기 배포 성공이다.

## 작업 원칙 — 사용자 지시 (2026-07-30) ★항상 적용
1. **폭넓게 분석해 최적안을 찾는다.** 지적받은 한 곳만 국소 수정하지 말고, 같은 문제가 있을 수 있는 범위를 전수 조사·측정해 근거를 만든 뒤, 대안을 비교해 최선을 택한다. 추정으로 바로 고치지 않는다.
2. **채팅 리포트는 기술 용어 없이, 쉽게 요약한다.** 코드·함수명·CSS 속성·파일 라인 같은 기술 내용은 채팅에 남기지 않는다(코드는 파일에, 상세 근거는 CLAUDE.md 체인지로그에). 채팅에는 "무엇이 문제였고 / 어떻게 바꿨고 / 사용자가 무엇을 체감하는지"만 비개발자가 읽을 수 있는 말로 쓴다.

## 점수·난이도 공정성 규칙 (2026-07-30) ★신규 게임 추가 시 필수
- **점수는 반드시 난이도 배수를 통과시킨다.** `dmScore(diff, pts, gameId)` 사용(`DMULT{easy:1, normal:1.4, hard:2}`). 그냥 `score+=10` 식으로 쓰면 어려움을 고른 사람이 손해를 보고, 최고 기록은 쉬움에만 쌓인다 — 34종 중 25곳이 이 버그였다.
- **난이도가 '판 크기'인 게임은 기본 배수로 보정되지 않는다.** 총점 규모 자체가 수십 배 차이나므로 시뮬레이션으로 난이도별 중앙값을 측정해 `DMULT_GAME`에 넣는다(merge = `{easy:1, normal:7, hard:70}`, `.logs/merge_sim.mjs` 400판 근거).
- **점수 눈금을 바꾸면 저장된 기록을 반드시 환산한다.** `migrateScales()`(1회성, `brain.scaleMigr` 플래그, 모든 게임 상태 const보다 앞에 위치). 환산하지 않으면 업데이트 직후 아무 판이나 신기록이 되고, `GAME_REF`가 함께 오른 게임은 능력치가 거꾸로 떨어져 보인다.
- **시간 고정 60초는 '반응형' 게임에만 맞다.** 난이도가 오를수록 해야 할 양이 늘어나는 완주형(단어찾기·카드짝·음높이·틀린그림)은 시간도 난이도별로 준다.
- 새 난이도 표를 만들면 hard가 normal과 같은 시작값이 아닌지 확인한다(rev/rhythm/pitch가 실제로 그랬다).

## 지속성·경쟁 구조 (2026-07-30)
- **결과 화면 공통 블록** = `afterGame(id, r)`. 훅 지점은 딱 둘: 아케이드 31종은 `missionMark`(반드시 `renderRecs`보다 **먼저**), 난이도 없는 5종은 `saveFlatBest`. 새 게임은 이 둘 중 하나만 통과하면 자동으로 붙는다. `PG_START`에 시작 함수명을 등록해야 '바로 다시'가 동작한다.
- **말로우 봇** = 게임별 목표치를 손으로 정하지 않는 자기 적응 사다리(`brain.bot.<id>`, 게임×난이도별). 내 첫 기록 ×1.05로 시드 → 이길 때마다 ×1.07. 34종 어디에 붙여도 튜닝 없이 아슬아슬한 상대가 된다. 0점 판에서는 시드하지 않는다(목표 1점짜리 허수 상대 방지).
- **주간 챌린지**(`brain.week`, 한국시간 월요일 리셋) = *지난 주의 나*와 대결. 점수는 **플레이했다는 사실**로만 오른다(한 판 10 / 개인기록 경신 +15 / 봇 승리 +10) — 게임별 임의 임계값 등급 금지 원칙(아래 2026-07-25)을 지키기 위한 설계다.
...
- **대안(안전)**: 태국 문화 기반 *비도박* 재미 콘텐츠 — 예) `สีมงคลประจำวันเกิด`(요일별 행운색), 태국 12띠·요일 부처. 기존 `재미(cat.fun)` 운세 계열에 그대로 들어가고 법·정책 리스크 0.
  → **2026-08-01 실제로 제작·배포함: `/luckycolor/`**(ko/en/th). 사실표·설계=`태국-행운색-기획.md`.
    유의: 태국 점성술의 하루는 **06시 시작**(0~5시 출생은 전날 요일), 수요일만 낮/밤 분리(밤=라후)라
    요일 7개·색 그룹 8개. 색표는 `CG` 순환 배열 하나에서 유도하며 **시작칸 바로 앞이 피할 색**.
