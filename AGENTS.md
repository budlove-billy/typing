...
- **Project:** brain
- **Working Directory:** D:\Codex\brain
- **Model:** Codex-opus-4-8

## Boundaries

1. **NEVER modify files outside your working directory** (`D:\Codex\brain`). You may read external files if needed, but all writes must stay within your project directory.
...
2. **Keep AGENTS.md updated** — append significant changes to the changelog at the bottom.
...
- 2026-09-07 — **말로우 운세 결과 공유 오류 수정**: 공유 콜백에서 존재하지 않는 `L.entry`를 참조하던 오류를 `LAST.entry`로 교체. 공유 API가 동기적으로 실패하거나 지원되지 않는 브라우저에서는 결과 PNG 다운로드로 폴백하고 Blob URL을 회수하도록 보강. 공유 API 호출·다운로드 링크 생성 및 320/390px smoke 검증 완료.
- 2026-09-08 — **홈 하단 게임 소개 가독성 개선**: 검색용 본문량은 유지하면서 긴 소개를 리드 문단·기능 안내 카드·FAQ 카드·면책으로 재구성. 행간·자간·강조·여백과 은은한 배경을 적용하고, 데스크톱 3열/모바일 1열 반응형으로 정리. ko/en/th 렌더와 320px 가로 넘침 0 확인.
...
Each chat turn spawns a fresh `Codex` subprocess. When the turn ends and the
...
- `roles add <code|key>` — install it as `./.Codex/agents/<name>.md`, then spawn it with the
...
A skill is a folder (SKILL.md + optional scripts) that Codex auto-loads by
...
- `skills add <code|key>` — **plug** it into `./.Codex/skills/<key>/` (Codex then
...
A Codex plugin bundles commands / sub-agents / skills / hooks / MCP servers. The
...
- 2026-09-22 — **수능 운세 결과 보완**: `unse/?from=suneung`에 기존 오늘의 총운·학업/일운·건강운을 바탕으로 한 재미용 집중 흐름·컨디션 흐름·오늘의 루틴 리포트를 추가했다. 시험 점수·등급 예측은 하지 않으며, 일반 운세 결과와 직접 유입 결과는 그대로 유지한다. 신년운세는 기존 `/newyear/` 랜딩과 `from=newyear` 문구 연결은 있으나 연간 결과 계산은 아직 없어 별도 연간 모드로 후속 적용한다.
...
- 2026-09-20 — **Mallow 메달 컬렉션 1차**: 기존 게임별 밸런스 기준(`GAME_REF`)에서 Bronze·Silver·Gold·Diamond 목표를 계산해, 결과 화면에 다음 메달까지의 거리와 Mallow 반응을 표시. 홈 컬렉션 요약과 내 기록 게임별 메달을 연결했으며, flash·spot·react·bubble·merge·sort에는 공통 깊이·집중 프레임을 적용. 기록 저장 형식 변경 없이 과거 최고점도 즉시 반영된다. 정적 계약·기록/미션 회귀·390px 메달 smoke 통과.
...
- 2026-09-08 — **웹 품질 기준선 자동화**: `tools/quality/`에 59개 HTML 스크립트 문법·게임 계약·번역 키·PWA 필수 자원을 검사하는 정적 검사와, ko/en/th에서 노출 게임 100개 조합을 실제 시작하고 320/390px 홈을 확인하는 브라우저 검사를 추가. 주석 처리된 `vocab`·`typing`을 집계하던 오류를 바로잡아 실제 노출 스킬 게임 34개로 홈 3개 언어·두뇌게임 총정리·연결 가이드의 표기를 통일. 통합 검사 전 항목 통과 후 배포.

- 2026-09-07 — **타로 3카드 결과 해석 개선**: 연애·일/학업·재물 결과에서 반복되던 “~운으로 본다면” 문구를 제거하고, 각 자리(과거·현재·미래)에 맞는 자연스러운 도입문과 카드 핵심 의미를 연결. 슈트·주제별 실천 힌트를 별도 스타일로 추가해 해석을 이해하기 쉽게 확장했으며, 태국어 결과도 동일한 구조로 보완. 78장×3주제×정/역 1,404개 조합 및 ko/th 모바일 결과 smoke 검증 완료.
