...
`.claudecrab/memory/facts/`, FTS-indexed. Read it in LAYERS — never bulk-read the facts
folder (a real project's 18 facts are ~14K tokens; the same set as one-liners is ~900):

- `memory brief` — EVERY fact as one line (what you know, cheaply). Start here when you
  need orientation; `--titles` for an even smaller list.
- `memory search "<q>"` — **do this FIRST** when a question touches past
  decisions/conventions/preferences; results show each fact's summary line.
- `memory show <slug>` — the FULL text, only for the facts a question actually landed on.
- `memory add "<title>" --body "<text>" [--tags a,b]` (one durable fact; specific
  titles) · `list` / `forget <slug>` / `reindex` · `import-legacy` (one-time: split an
  overgrown MEMORY.md's ## sections into facts). MEMORY.md past ~60 lines → import-legacy + trim.
<!-- MEMORY_TOOL_END -->